...
class PhantomJsDownloader {
    ...
    public File downloadAndExtract() {
        ...
        File phantomJsExe;
        if (isWindows) {
            url = "http://phantomjs.googlecode.com/files/phantomjs-1.9.0-windows.zip";
            phantomJsExe = new File(installDir, "phantomjs-1.9.0-windows/phantomjs.exe");
        } else if (isMac) {
            url = "http://phantomjs.googlecode.com/files/phantomjs-1.9.0-macosx.zip";
            phantomJsExe = new File(installDir, "phantomjs-1.9.0-macosx/bin/phantomjs");
        } else if (isLinux64) {
    ...
    private void extractExe(String url, File phantomInstallDir, File phantomJsExe) {
        ...
        try {
            if (isWindows) {
                ...
                new ProcessBuilder().command("/usr/bin/unzip", "-qo", "phantomjs.zip").directory(phantomInstallDir).start()
                        .waitFor();
            } else {
